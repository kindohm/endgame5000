...
s.waitForBoot {
	...
	MIDIClient.sources.do({ arg item, i;
		...
		if (item.name == "Bus 1", {
			~synth1Out = MIDIOut.newByName("IAC Driver", "Bus 1");
			~synth1Out.latency = 0;
			~dirt.soundLibrary.addMIDI(\synth1, ~synth1Out);
			"😍 added synth1".postln;
		});

		if (item.name == "Bus 2", {
			~synth2Out = MIDIOut.newByName("IAC Driver", "Bus 2");
			~synth2Out.latency = 0;
			~dirt.soundLibrary.addMIDI(\synth2, ~synth2Out);
			"😍 added synth2".postln;
		});

		if (item.name == "Bus 3", {
			~synth3Out = MIDIOut.newByName("IAC Driver", "Bus 3");
			~synth3Out.latency = 0;
			~dirt.soundLibrary.addMIDI(\drums, ~synth3Out);
			"😍 added drums".postln;
		});
		...
		// 		// chan.postln;
		// 		// "debug1: ".catArgs(chan).catArgs(" ").catArgs(num).catArgs(" ").catArgs(val).postln;
		// 		// if (chan == 1, {
